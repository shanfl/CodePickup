...
int CFont::SetSize(float x, float y)       // Set the size of the font
{
  sizex=x;
  sizey=y;
...
int CFont::BuildFont(char *file)          // Build Our Font Display List
{
  int loop1;
  float cx, cy;

  if(!font.LoadTGA(file))
  {
    return 1;
  }

  CLog::addLine2("Building font ....");
...
int CFont::KillFont(GLvoid)                          // Delete The Font From Memory
{
  CLog::addLine2("Killing the font ....");
  glDeleteLists(base,256);                       // Delete All 256 Display Lists
  CLog::addLine("....... done");
  return 1;
}

int CFont::PrintText(GLint x, GLint y, char type, char *fmt,...)// Where The Printing Happens
{
