...
// We added to functions to this math file:  PlaneDistance() and IntersectPlane().
...
{
	VECTOR3D vVector = {0};								// Initialize our variable to zero
...
{														// Get 2 vectors from the polygon (2 sides), Remember the order!
	VECTOR3D vVector1 = Vector(vTriangle[2], vTriangle[0]);
	VECTOR3D vVector2 = Vector(vTriangle[1], vTriangle[0]);

	VECTOR3D vNormal = Cross(vVector1, vVector2);		// Take the cross product of our 2 vectors to get a perpendicular vector
...
// this so we had the choice of getting the normal and distance back, or not.
// I also changed the vTriangle to "vPoly" because it isn't always a triangle.
// The code doesn't change, it's just more correct (thought we only need 3 points anyway).
...
{
	VECTOR3D vPoint = {0}, vLineDir = {0};		// Variables to hold the point and the line's direction
...
bool IntersectedPolygon(VECTOR3D vPoly[], VECTOR3D vLine[], int verticeCount)
{
	...
	VECTOR3D vIntersection = IntersectionPoint(vNormal, vLine, originDistance);
